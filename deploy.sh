...
import subprocess
sha=subprocess.run(["git","-C","/home/danny/w6-pings","rev-parse","HEAD"],capture_output=True,text=True).stdout.strip()
d=requests.post(f"https://api.render.com/v1/services/{S}/deploys",headers=H,json={"clearCache":"do_not_clear","commitId":sha}).json()
